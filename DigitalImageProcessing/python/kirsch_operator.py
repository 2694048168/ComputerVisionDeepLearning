...
"""
@Funciton: Kirsch 边缘检测算子 —— 四面八方的进行差分(梯度信息，边缘强度)
@Python Version: 3.8
@Author: Wei Li
@Date: 2021-10-16
"""
...
def KirschOperator(image, _boundary="fill", _fillvalue=0):
    """函数 KirschOperator 实现图像与每一个核的卷积，
    ...
    kernel_2 = np.array([[-3, -3, -3], [-3, 0, -3], [5, 5, 5]])
    ...
    kernel_3 = np.array([[-3, 5, 5], [-3, 0, 5], [-3, -3, -3]])
    ...
    kernel_4 = np.array([[-3, -3, -3], [5, 0, -3], [5, 5, -3]])
    ...
    kernel_5 = np.array([[-3, -3, 5], [-3, 0, 5], [-3, -3, 5]])
    ...
    kernel_6 = np.array([[5, -3, 5], [5, 0, -3], [5, -3, -3]])
    ...
    kernel_7 = np.array([[-3, -3, -3], [-3, 0, 5], [-3, 5, 5]])
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        # 边缘强度
        edge = KirschOperator(image, _boundary="symm")
        # 边缘强度的灰度级显示
        edge[edge>255] = 255
