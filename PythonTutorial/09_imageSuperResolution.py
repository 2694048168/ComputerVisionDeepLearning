...
class SRCNN(torch.nn.Module):
    def __init__(self) -> None:
        ...
        self.features = torch.nn.Sequential(
            torch.nn.Conv2d(1, 64, (9, 9), (1, 1), (4, 4)), torch.nn.ReLU(True)
        )

        # Non-linear mapping layer.
        self.map = torch.nn.Sequential(
            torch.nn.Conv2d(64, 32, (5, 5), (1, 1), (2, 2)), torch.nn.ReLU(True)
        )
...
class ImageSR:
    ...
    def readImageFolder(self):
        ...
        for extension in ["png", "jpg", "jpeg", "bmp", "tiff"]:
            self.m_image_list += glob.glob(
                os.path.join(self.m_original_folder, f"*.{extension}")
            )
        assert len(self.m_image_list), (
            f"there is not any image in the {self.m_original_folder}!"
        )
    ...
    def inference(self):
        ...
        model = SRCNN()
        model = model.to(
            memory_format=torch.channels_last, device=torch.device("cuda", 0)
        )
        ...
        sr_image = model.eval()
        filepath = f"{self.m_save_folder}/sr_img.png"
        cv2.imwrite(filepath, sr_image * 255.0)
        print(f"SR image save to {filepath}")
